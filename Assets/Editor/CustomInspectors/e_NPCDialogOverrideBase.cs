...
[CustomEditor(typeof(NPCDialogOverrideBase), editorForChildClasses:true)]
public class e_NPCDialogOverrideBase : Editor {

	private ReorderableList list;
	private NPCDialogOverrideBase script;
	private NPCController other;

	void OnEnable() {
		script = target as NPCDialogOverrideBase;
		other = script.GetComponent<NPCController>();
	...
	public override void OnInspectorGUI() {
		...
		if (GUILayout.Button("Steal dialog tree") && script != null && other != null && other.dialogs != null) {
			// Copy the list
			Undo.RecordObject(target, "Steal dialog tree");

			script.dialogs = NPCController.CopyDialog(other.dialogs);
		}
